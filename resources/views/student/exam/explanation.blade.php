...
@section('content')
<div x-data="reviewShell()" x-init="init()" class="fixed inset-0 bg-slate-50 z-[50] flex flex-col overflow-hidden select-none">
    {{-- Top Bar Navigation --}}
    <div class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 relative z-[60]">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider leading-none">Pembahasan Soal</h2>
                <p class="text-[9px] text-slate-400 font-semibold uppercase tracking-wider mt-1">{{ $session->title }}</p>
            </div>
            <a href="{{ route('student.exam.results', $examResult) }}" class="ml-2 px-3 py-1.5 border border-slate-200 rounded-lg text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition-all flex items-center gap-1.5">
    ...
    <div class="flex-1 flex overflow-hidden">
        ...
        <template x-if="!allQuestions || allQuestions.length === 0">
            <div class="flex-1 flex flex-col items-center justify-center text-center bg-white p-8">
                ...
                <p class="text-xs text-slate-450 mt-1 max-w-xs leading-relaxed">Belum ada soal tryout yang ditambahkan pada subtest yang telah diselesaikan.</p>
                <a href="{{ route('student.exam.results', $examResult) }}" class="mt-6 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider shadow-sm">Kembali ke Hasil</a>
            </div>
        </template>
                    ...
                    <div class="max-w-3xl mx-auto space-y-8 pb-24">
                        ...
                        <div class="space-y-4">
                            <div class="flex items-center gap-2">
                                 <span class="text-[9px] font-bold text-blue-700 bg-blue-50 px-2 py-0.5 rounded tracking-wide uppercase border border-blue-100/50" x-text="currentQuestion ? currentQuestion.subtest_title : ''"></span>
                                 <span class="text-[10px] font-semibold text-slate-400 uppercase">Soal #<span x-text="currentIndex + 1"></span></span>
                            </div>

                            <div class="flex items-start gap-4">
                                <div class="w-8 h-8 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center shrink-0">
                                    <span class="text-xs font-bold text-slate-700" x-text="currentIndex + 1">1</span>
                                </div>
                                <div class="flex-1 prose prose-slate max-w-none text-slate-800 text-sm font-medium leading-relaxed exam-question-content" x-html="currentQuestion ? currentQuestion.content : ''"></div>
                            </div>
                        </div>
                                    ...
                                    <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                                        <div class="relative flex items-center p-3.5 rounded-xl border transition-all"
                                            ...
                                            <div class="absolute -right-2 -top-2 flex gap-1 z-10">
                                                <span x-show="option.is_correct" class="px-2 py-0.5 bg-emerald-600 text-white text-[8px] font-bold uppercase rounded-md shadow-xs">Kunci Jawaban</span>
                                                <span x-show="userChoiceId == option.id" 
                                                      class="px-2 py-0.5 text-white text-[8px] font-bold uppercase rounded-md shadow-xs"
                                                      :class="option.is_correct ? 'bg-blue-600' : 'bg-rose-600'">Jawaban Anda</span>
                                            </div>
                                            ...
                                            <div class="flex-1 text-xs font-semibold leading-relaxed" :class="option.is_correct ? 'text-emerald-900' : (userChoiceId == option.id ? 'text-rose-900' : 'text-slate-700')" x-html="option.content"></div>

                                            {{-- Icon Indicator --}}
                                            <div class="ml-4 shrink-0">
                                                <template x-if="option.is_correct">
                                                    <svg class="w-4 h-4 text-emerald-650" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <div class="space-y-3">
                                    <template x-for="(option, index) in currentQuestion.options" :key="'mc-'+option.id">
                                        <div class="relative flex items-center p-3.5 rounded-xl border transition-all bg-white border-slate-200"
                                            ...
                                            <div class="flex-1 text-xs font-semibold text-slate-700 leading-relaxed" x-html="option.content"></div>
                                            <div class="flex items-center gap-2">
                                                <template x-if="option.is_correct">
                                                    <span class="px-2 py-0.5 bg-emerald-50 border border-emerald-200 text-emerald-700 text-[8px] font-bold uppercase rounded">Kunci</span>
                                                </template>
                                                <template x-if="selectedOptionIds.includes(option.id)">
                                                    <span class="px-2 py-0.5 text-white text-[8px] font-bold uppercase rounded"
                                                          :class="option.is_correct ? 'bg-blue-600' : 'bg-rose-600'">Pilihan Anda</span>
                                                </template>
                                            </div>
                                        </div>
                                    </template>
                                    ...
                                    <table class="w-full text-left border-collapse">
                                        <thead>
                                            <tr class="bg-slate-50 border-b border-slate-200">
                                                ...
                                                <th class="px-5 py-3 text-[9px] font-bold text-slate-500 uppercase tracking-wider">Pilihan Anda</th>
                                        ...
                                        <tbody class="divide-y divide-slate-150">
                                            <template x-for="option in currentQuestion.options" :key="'match-row-'+option.id">
                                                <tr class="hover:bg-slate-50/30 transition-colors">
                                                    <td class="px-5 py-3 text-xs font-semibold text-slate-650" x-html="option.label"></td>
                                                    <td class="px-5 py-3">
                                                        <div x-show="matchingAnswers[option.id]" class="px-2.5 py-1 rounded text-[9px] font-bold uppercase inline-block border"
                                                    ...
                                                    <td class="px-5 py-3">
                                                        <svg x-show="matchingAnswers[option.id] == option.id" class="w-4.5 h-4.5 text-emerald-500" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
                                                        </svg>
                                                        <svg x-show="matchingAnswers[option.id] && matchingAnswers[option.id] != option.id" class="w-4.5 h-4.5 text-rose-500" fill="currentColor" viewBox="0 0 20 20">
                                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
                                                        </svg>
                                                    </td>
                                                </tr>
                            ...
                            <template x-if="currentQuestion && currentQuestion.type === 'essai'">
                                <div class="space-y-3">
                                    <div class="p-4 bg-white border border-slate-200 rounded-xl shadow-xs">
                                        ...
                                        <div class="text-xs font-semibold text-slate-700 leading-relaxed whitespace-pre-wrap" x-text="essayAnswer || 'Tidak ada jawaban.'"></div>
                                    </div>
                                </div>
                            </template>
                        </div>
                        ...
                        <div class="pl-12 scroll-mt-20" id="explanation-box" x-show="currentQuestion">
                            <div class="p-6 bg-slate-900 rounded-xl text-white space-y-3.5 shadow-sm">
                                ...
                                <div class="prose prose-invert prose-sm max-w-none text-slate-300 text-xs font-medium leading-relaxed" x-show="currentQuestion && currentQuestion.explanation" x-html="currentQuestion ? currentQuestion.explanation : ''"></div>
                                <div class="text-xs font-semibold text-slate-500 italic" x-show="currentQuestion && !currentQuestion.explanation">Belum ada pembahasan tertulis untuk soal ini.</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="w-80 bg-white border-l border-slate-200 flex flex-col shrink-0 no-print">
                    ...
                    <div class="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-6">
                        @foreach($subtests as $subtest)
                            <div class="space-y-2">
                                 <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider pl-1">{{ $subtest->title ?: ($subtest->subject->name ?? 'Subtest') }}</p>
                    ...
                    <div class="p-5 bg-slate-50 border-t border-slate-200 no-print">
                         <div class="grid grid-cols-2 gap-3">
                            <button @click="prevQuestion()" :disabled="currentIndex === 0" class="py-2.5 bg-white border border-slate-200 text-xs font-bold uppercase tracking-wider rounded-lg text-slate-500 enabled:hover:bg-slate-50 hover:text-slate-700 transition-all disabled:opacity-40 shadow-xs">Sebelumnya</button>
                            <button @click="nextQuestion()" :disabled="currentIndex === allQuestions.length - 1" class="py-2.5 bg-white border border-slate-200 text-xs font-bold uppercase tracking-wider rounded-lg text-slate-500 enabled:hover:bg-slate-50 hover:text-slate-700 transition-all disabled:opacity-40 shadow-xs">Selanjutnya</button>
                         </div>
                         <button onclick="window.print()" class="w-full mt-3 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm">
...
<style>
    ...
    @media print {
        ...
        #explanation-box { box-shadow: none !important; background: #f8fafc !important; page-break-inside: avoid; }
            ...
            getNavClass(q) {
                ...
                if (ans) {
                    ...
                    if (q.type === 'pilihan_ganda') {
                        ...
                        let correctCount = 0;
                        q.options.forEach(o => {
                            if (matches[o.id] == o.id) correctCount++;
                        });
