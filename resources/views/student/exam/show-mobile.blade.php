...
<div x-data="examShell()" x-init="initExam()" class="space-y-6 pb-28">
    ...
    <div x-show="isTransitioning" class="fixed inset-0 z-[120] bg-slate-900/95 flex flex-col items-center justify-center text-center p-6 text-white" style="display: none;">
        <div class="w-16 h-16 bg-amber-500/10 text-amber-500 border border-amber-500/20 rounded-full flex items-center justify-center mb-6 animate-pulse">
            <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path>
            </svg>
        </div>
        <h3 class="text-lg font-extrabold tracking-tight">Waktu Subtest Berakhir!</h3>
        <p class="text-xs text-slate-400 mt-2 max-w-xs leading-relaxed">Jawaban Anda telah disimpan. Sistem akan mengarahkan Anda ke subtest berikutnya secara otomatis dalam:</p>
        <span class="text-5xl font-black text-amber-500 mt-6 block tabular-nums" x-text="transitionSeconds">10</span>
        <p class="text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-8">Mohon tidak menutup halaman ini</p>
    </div>
    ...
    <div class="bg-slate-900 rounded-2xl p-4 text-white shadow-md flex items-center justify-between gap-4">
        <div class="min-w-0">
            ...
            <span class="text-xs font-bold text-slate-100 truncate block mt-0.5" x-text="currentQuestion ? currentQuestion.subtest_title : '{{ $currentSubtest->title ?? 'Subtest' }}'"></span>
        </div>
        ...
        <div class="px-3.5 py-1.5 bg-white/10 rounded-xl flex items-center gap-2 border border-white/5">
            ...
            <span class="text-xs font-black text-white tabular-nums tracking-wider" x-text="formatTime(remainingSeconds)">00:00</span>
        </div>
    </div>
    ...
    <div class="space-y-4" x-show="!isTransitioning && currentQuestion">
        ...
        <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
            <div class="flex items-center justify-between gap-3">
                <span class="px-2 py-0.5 bg-indigo-50 border border-indigo-150 rounded text-[9px] font-extrabold text-indigo-600 uppercase tracking-wider">
                    Siswa Portal
                </span>
                <span class="text-[10px] font-bold text-slate-400 uppercase">Soal #<span x-text="currentIndex + 1"></span> dari <span x-text="allQuestions.length"></span></span>
            </div>

            <div class="flex items-start gap-3 border-t border-slate-50 pt-4">
                <div class="w-7 h-7 rounded-lg border border-slate-200 bg-slate-50 flex items-center justify-center shrink-0">
                    <span class="text-xs font-black text-slate-700" x-text="currentIndex + 1">1</span>
                </div>
                <div class="flex-1 prose prose-slate max-w-none text-slate-800 text-xs font-semibold leading-relaxed exam-question-content" x-html="currentQuestion ? currentQuestion.content : ''"></div>
            </div>
        </div>
        ...
        <div class="space-y-3">
            ...
            <template x-if="currentQuestion && currentQuestion.type === 'pilihan_ganda'">
                <div class="space-y-3">
                    <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                        <label class="relative flex items-center p-4 rounded-xl border border-slate-200 cursor-pointer transition-all active:bg-slate-50 group bg-white shadow-xs"
            ...
            <template x-if="currentQuestion && currentQuestion.type === 'pilihan_ganda_kompleks'">
                <div class="space-y-3">
                    <p class="text-[9px] font-bold text-indigo-600 uppercase tracking-widest pl-1 flex items-center gap-1.5">
                    ...
                    <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                        <label class="relative flex items-center p-4 rounded-xl border border-slate-200 cursor-pointer transition-all active:bg-slate-50 bg-white shadow-xs"
                               :class="selectedOptionIds.includes(option.id) ? 'border-indigo-600 ring-2 ring-indigo-50 bg-indigo-50/10' : 'bg-white'">
                            <input type="checkbox" :value="option.id" x-model="selectedOptionIds" @change="saveAnswer()" class="w-4.5 h-4.5 text-indigo-600 border-slate-300 rounded focus:ring-indigo-100 mr-3 transition-all">
                            <div class="flex-1 text-xs font-semibold text-slate-700 leading-relaxed" x-html="option.content"></div>
                        </label>
                    </template>
                </div>
            ...
            <template x-if="currentQuestion && currentQuestion.type === 'menjodohkan'">
                <div class="space-y-4">
                    <div class="p-3.5 bg-indigo-50/50 border border-indigo-100/50 rounded-xl flex items-start gap-2.5">
                        <svg class="w-4.5 h-4.5 text-indigo-600 shrink-0 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-3">
                        ...
                        <template x-for="(option, index) in currentQuestion.options" :key="'premise-'+option.id">
                            <div class="flex items-center gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs min-h-[60px]">
                                <div class="flex-1 text-xs font-semibold text-slate-650 leading-normal" x-html="option.label"></div>
                                ...
                                <div class="flex-1 min-h-[44px] p-1.5 rounded-lg bg-slate-50 border-2 border-dashed border-slate-200 matching-drop-target transition-all duration-200 flex items-center justify-center"
                                    ...
                                    <template x-if="matchingAnswers[option.id]">
                                        <div class="w-full bg-indigo-600 text-white py-1.5 px-2.5 rounded-md text-[10px] font-bold uppercase text-center shadow-xs cursor-move flex items-center justify-between gap-1"
                                            ...
                                            <button @click.stop="unmatch(option.id)" class="text-white/80 hover:text-white">
                                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="space-y-2 p-4 bg-slate-50 rounded-xl border border-slate-200/80">
                        <h4 class="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-center mb-1">Pilihan Jawaban</h4>
            ...
            <template x-if="currentQuestion && currentQuestion.type === 'essai'">
                <div class="space-y-3">
                     <div class="p-4 bg-white rounded-xl border border-slate-200 focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-50 transition-all">
    ...
    <div x-show="showQuestionsDrawer" class="fixed inset-0 z-50 overflow-hidden" style="display: none;">
        {{-- Backdrop --}}
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity" @click="showQuestionsDrawer = false"></div>
        
        {{-- Sheet --}}
        <div class="fixed bottom-0 inset-x-0 bg-white rounded-t-2xl border-t border-slate-200 max-h-[70vh] flex flex-col z-10 overflow-hidden transform transition-all duration-300">
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-center gap-4 text-[9px] font-bold uppercase tracking-wider text-slate-500">
                <div class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded bg-emerald-500"></span> Terjawab</div>
                <div class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded bg-amber-500"></span> Ragu-Ragu</div>
                <div class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded bg-white border border-slate-200">Kosong</span></div>
            </div>
        </div>
    </div>
    ...
    <div x-show="showFinishModal" class="fixed inset-0 z-50 overflow-hidden" style="display: none;">
        <div class="fixed inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity" @click="showFinishModal = false"></div>
        <div class="fixed bottom-0 inset-x-0 bg-white rounded-t-2xl border-t border-slate-200 z-10 p-5 space-y-4 text-center animate-in slide-in-from-bottom duration-350">
            <div class="w-11 h-11 bg-rose-50 text-rose-600 rounded-full flex items-center justify-center mx-auto border border-rose-100 shadow-sm">
            ...
            <div class="space-y-1">
                ...
                <p class="text-[11px] text-slate-500 max-w-xs mx-auto leading-relaxed">Pastikan semua soal telah terjawab. Setelah selesai, Anda tidak dapat mengubah jawaban lagi.</p>
            </div>

            <div class="flex gap-2 pt-2">
                <button @click="showFinishModal = false" class="flex-1 py-2.5 bg-slate-50 hover:bg-slate-100 border border-slate-200 text-slate-650 rounded-lg text-xs font-bold uppercase tracking-wider transition-all">
                    Batal
                </button>
                <form action="{{ route('student.exam.finish', $examResult) }}" method="POST" class="flex-1">
                    @csrf
                    <button type="submit" class="w-full py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm">
                        Selesai & Kirim
                    </button>
                </form>
            </div>
