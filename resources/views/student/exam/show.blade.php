...
<div x-data="examShell()" x-init="initExam()" class="fixed inset-0 bg-slate-50 z-[50] flex flex-col overflow-hidden select-none">
    {{-- Top Bar Navigation --}}
    <div class="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 relative z-[60]">
        <div class="flex items-center gap-4">
            ...
            <div class="hidden sm:block">
                <h2 class="text-xs font-bold text-slate-800 uppercase tracking-wider leading-none">{{ $session->title }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="text-[9px] text-blue-700 font-bold uppercase tracking-wider bg-blue-50 px-1.5 py-0.5 rounded border border-blue-100/50">SUBTEST: {{ $currentSubtest ? ($currentSubtest->title ?? ($currentSubtest->subject->name ?? 'Subtest')) : 'Subtest' }}</span>
                </div>
            </div>
            <a href="{{ route('dashboard') }}" class="ml-2 px-3 py-1.5 border border-slate-200 rounded-lg text-[10px] font-semibold uppercase tracking-wider text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition-all flex items-center gap-1.5">
        ...
        <div class="flex items-center gap-3">
             ...
             <div class="hidden lg:flex items-center gap-2 mr-4">
                <template x-for="(st, index) in allSubtests" :key="st.id">
                    <div class="flex items-center">
                        ...
                        <div x-show="index < allSubtests.length - 1" class="w-4 h-0.5" :class="completedSubtests.includes(st.id) ? 'bg-emerald-200' : 'bg-slate-100'"></div>
                    </div>
                </template>
             </div>
             ...
             <div class="px-4 py-1.5 bg-slate-900 rounded-lg flex items-center gap-2.5">
                ...
                <div class="flex flex-col">
                    <span class="text-[8px] font-bold text-blue-300 uppercase tracking-wider leading-none mb-0.5">Sisa Waktu</span>
                    <span class="text-xs font-bold text-white tabular-nums leading-none tracking-wider" x-text="formatTime(remainingSeconds)">00:00:00</span>
                </div>
            </div>

             @if($currentSubtest && $allSubtests->last()->id == $currentSubtest->id)
                <button @click="finishExam()" class="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm">
                    Selesai
                </button>
             @endif
        </div>
    ...
    <div class="flex-1 flex overflow-hidden">
            ...
            <div class="max-w-3xl mx-auto space-y-6 pb-24">
                ...
                <template x-if="!allQuestions || allQuestions.length === 0">
                    <div class="flex flex-col items-center justify-center py-20 text-center">
                        <div class="w-12 h-12 bg-slate-50 text-slate-400 rounded-xl flex items-center justify-center mb-4 border border-slate-200">
                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9.172 16.172a4 4 0 015.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                            </svg>
                        </div>
                        <h4 class="text-sm font-semibold text-slate-800">Belum Ada Soal</h4>
                        <p class="text-xs text-slate-450 mt-1 max-w-xs leading-relaxed">Admin belum menambahkan daftar soal pada subtest ini.</p>
                        <a href="{{ route('dashboard') }}" class="mt-6 px-4 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold uppercase tracking-wider">Kembali ke Dashboard</a>
                    </div>
                </template>
                ...
                <div class="flex items-start gap-4" x-show="!isTransitioning && currentQuestion">
                    ...
                    <div class="flex-1 min-w-0">
                         <div class="prose prose-slate max-w-none text-slate-800 text-sm font-medium leading-relaxed exam-question-content" x-html="currentQuestion ? currentQuestion.content : ''"></div>
                    </div>
                </div>
                ...
                <div class="pl-12" x-show="!isTransitioning && currentQuestion">
                    ...
                    <template x-if="currentQuestion && currentQuestion.type === 'pilihan_ganda'">
                        <div class="space-y-3">
                            <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                                <label class="relative flex items-center p-3.5 rounded-xl border border-slate-200 cursor-pointer transition-all hover:bg-slate-50/50 hover:border-blue-400 group"
                                    ...
                                    <div class="flex-1 text-xs font-semibold text-slate-750 leading-relaxed" x-html="option.content"></div>
                                </label>
                            </template>
                        </div>
                    </template>
                    ...
                    <template x-if="currentQuestion && currentQuestion.type === 'pilihan_ganda_kompleks'">
                        <div class="space-y-3">
                            <p class="text-[10px] font-bold text-blue-600 uppercase tracking-wider mb-2 flex items-center gap-1.5">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                                <label class="relative flex items-center p-3.5 rounded-xl border border-slate-200 cursor-pointer transition-all hover:bg-slate-50/50 hover:border-blue-400 group"
                                       :class="selectedOptionIds.includes(option.id) ? 'bg-blue-50/30 border-blue-500 ring-2 ring-blue-50' : 'bg-white shadow-xs'">
                                    <input type="checkbox" :value="option.id" x-model="selectedOptionIds" @change="saveAnswer()" class="w-4 h-4 text-blue-600 border-slate-300 rounded focus:ring-blue-100 mr-4 transition-all">
                                    <div class="flex-1 text-xs font-semibold text-slate-750 leading-relaxed" x-html="option.content"></div>
                                </label>
                            </template>
                        </div>
                    ...
                    <template x-if="currentQuestion && currentQuestion.type === 'menjodohkan'">
                        <div class="space-y-6">
                            <div class="p-4 bg-blue-50/40 border border-blue-100 rounded-xl flex items-center gap-3">
                                <div class="w-7 h-7 bg-blue-100 text-blue-600 rounded-lg flex items-center justify-center shrink-0 border border-blue-200/50">
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                                ...
                                <div class="space-y-3">
                                    <template x-for="(option, index) in currentQuestion.options" :key="'premise-'+option.id">
                                        <div class="flex items-center gap-3 bg-white p-3.5 rounded-xl border border-slate-200 shadow-xs min-h-[70px]">
                                            <div class="flex-1 text-xs font-semibold text-slate-600 leading-normal" x-html="option.label"></div>
                                            <div class="w-6 flex items-center justify-center shrink-0">
                                                <svg class="w-4 h-4 text-slate-350" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            ...
                                            <div class="flex-1 min-h-[50px] p-1.5 rounded-lg bg-slate-50 border-2 border-dashed border-slate-200 matching-drop-target transition-all duration-200 flex items-center justify-center"
                                                ...
                                                <template x-if="matchingAnswers[option.id]">
                                                    <div class="w-full bg-blue-600 text-white p-2 rounded-md text-[10px] font-bold uppercase text-center shadow-xs cursor-move flex items-center justify-between gap-1.5"
                                                        ...
                                                        <button @click.stop="unmatch(option.id)" class="hover:text-red-200 transition-colors">
                                ...
                                <div class="space-y-3 p-4 bg-slate-50 rounded-2xl border border-slate-200">
                                    <h4 class="text-[9px] font-bold text-slate-400 uppercase tracking-wider text-center mb-2">Pilihan Jawaban (Tarik Ke Kiri)</h4>
                                    <div class="space-y-2 matching-pool min-h-[80px]" x-init="initSortable($el)">
                                        ...
                                        <template x-if="getAvailableMatches().length === 0">
                                            <div class="py-8 flex flex-col items-center justify-center opacity-40">
                                                <svg class="w-6 h-6 text-slate-350 mb-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path>
                                                </svg>
                                                <p class="text-[9px] font-semibold text-slate-400 uppercase tracking-wider italic">Terpasang Semua</p>
                                            </div>
                                        </template>
                                    </div>
                                </div>
                    ...
                    <template x-if="currentQuestion && currentQuestion.type === 'essai'">
                        <div class="space-y-3 max-w-2xl">
                             <div class="p-4 bg-white rounded-2xl border border-slate-200 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
                             ...
                             <div class="flex items-center gap-2 text-slate-400">
                                ...
                                <span class="text-[9px] font-medium tracking-wide italic">Jawaban akan tersimpan otomatis saat Anda mengetik</span>
                             </div>
                        </div>
                    </template>
                </div>
                ...
                <div class="pt-8 border-t border-slate-150 flex items-center justify-between gap-4" x-show="!isTransitioning && currentQuestion">
        ...
        <div class="w-80 bg-white border-l border-slate-200 flex flex-col shrink-0">
            <div class="p-5 border-b border-slate-200 bg-slate-50/30">
                ...
                <p class="text-[10px] text-slate-400 font-semibold uppercase tracking-wider mt-0.5">Hanya Subtest Berjalan</p>
            </div>
            ...
            <div class="flex-1 overflow-y-auto p-5 custom-scrollbar space-y-6">
                ...
                <div class="p-4 bg-slate-50 rounded-xl border border-slate-200/80">
                    <div class="flex items-center justify-between mb-2">
                         <span class="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Progres Subtest</span>
                         <span class="text-xs font-bold text-slate-800" x-text="Math.round((Object.keys(answers).length / allQuestions.length) * 100) + '%'">0%</span>
                    </div>
                    <div class="w-full bg-slate-200 h-1.5 rounded-full overflow-hidden">
                        <div class="bg-blue-600 h-full transition-all duration-300" :style="'width: ' + ((Object.keys(answers).length / allQuestions.length) * 100) + '%'"></div>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div x-show="isTransitioning" x-transition.opacity class="fixed inset-0 z-[100] bg-slate-950/95 backdrop-blur-md flex items-center justify-center p-6 text-center">
        <div class="max-w-md w-full space-y-8">
            {{-- Modern Circular Progress Timer --}}
            <div class="w-32 h-32 border-4 border-slate-800 rounded-full flex flex-col items-center justify-center mx-auto relative bg-slate-900 shadow-xl">
                {{-- Rotating Elegant Progress Arc --}}
                <div class="absolute -inset-1.5 rounded-full border-2 border-t-blue-500 border-r-transparent border-b-transparent border-l-transparent animate-spin [animation-duration:3s]"></div>
                ...
                <div class="z-10 flex flex-col items-center justify-center animate-[pulse_2s_infinite]">
                    <span class="text-4xl font-bold text-white tabular-nums tracking-tight" x-text="transitionSeconds">10</span>
                    <span class="text-[8px] font-bold text-slate-400 uppercase tracking-widest mt-1">Detik</span>
                </div>
            </div>
            
            <div class="space-y-2">
                <h3 class="text-lg font-bold text-white uppercase tracking-wider" x-text="remainingSeconds <= 0 ? 'Waktu Subtest Selesai' : 'Lanjut ke Subtest Berikutnya'"></h3>
            ...
            <div class="p-4 bg-slate-900/60 rounded-xl border border-slate-800/80 text-left flex items-start gap-3.5">
                <div class="w-8 h-8 bg-blue-900/40 text-blue-400 rounded-lg flex items-center justify-center shrink-0 border border-blue-800/30">
                    <svg class="w-4.5 h-4.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M9 12l2 2 4-4"></path>
                    </svg>
                </div>
                <p class="text-xs text-slate-300 leading-relaxed font-medium">
                    Jangan khawatir, semua jawaban Anda pada subtest sebelumnya telah **tersimpan dengan aman** di server. Harap tetap tenang dan tunggu hitungan mundur selesai.
                </p>
            </div>
        </div>
    </div>
            ...
            <div class="p-6 text-center pt-8">
                ...
                <div class="grid grid-cols-2 gap-3 mt-6">
                    <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-center">
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-2">Terjawab</p>
                        <p class="text-base font-bold text-slate-800 leading-none" x-text="Object.keys(answers).length"></p>
                    </div>
                    <div class="p-3.5 bg-slate-50 rounded-xl border border-slate-200 text-center">
                        <p class="text-[9px] font-bold text-slate-400 uppercase tracking-wider leading-none mb-2">Kosong</p>
                        <p class="text-base font-bold text-slate-600 leading-none" x-text="totalExamQuestions - Object.keys(answers).length"></p>
                    </div>
                </div>
            </div>
            <div class="p-4 bg-slate-50 border-t border-slate-200 flex gap-3">
                 <button @click="showFinishModal = false" class="flex-1 py-2.5 bg-white border border-slate-200 text-slate-500 rounded-lg text-xs font-bold uppercase tracking-wider hover:bg-slate-100 transition-all">
                    Batal
                </button>
                <form action="{{ route('student.exam.finish', $examResult) }}" method="POST" class="flex-1 flex">
                    @csrf
                    <button type="submit" class="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-sm">
                        Ya, Selesai!
                    </button>
                </form>
            </div>
...
<style>
    .exam-question-content img { max-width: 100%; height: auto; border-radius: 0.5rem; margin: 1rem 0; }
    .exam-question-content table { border-collapse: collapse; margin: 1rem 0; width: 100%; }
    .exam-question-content td, .exam-question-content th { border: 1px solid #e2e8f0; padding: 0.5rem; font-size: 0.8rem; }
