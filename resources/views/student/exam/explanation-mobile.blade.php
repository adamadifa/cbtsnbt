...
<div x-data="reviewShell()" x-init="init()" class="space-y-6 pb-28">
    ...
    <template x-if="!allQuestions || allQuestions.length === 0">
        <div class="flex flex-col items-center justify-center text-center bg-white p-8 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <a href="{{ route('student.exam.results', $examResult) }}" class="mt-6 px-4 py-2 bg-indigo-650 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold uppercase tracking-wider shadow-sm">Kembali</a>
        </div>
    </template>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-5 shadow-xs space-y-4">
                <div class="flex items-center justify-between gap-3">
                    <span class="text-[9px] font-bold text-indigo-700 bg-indigo-50 px-2.5 py-0.5 rounded tracking-wide uppercase border border-indigo-100" x-text="currentQuestion ? currentQuestion.subtest_title : ''"></span>
                        ...
                        <template x-for="(option, index) in currentQuestion.options" :key="option.id">
                            <div class="relative flex items-center p-4 rounded-xl border transition-all"
                                ...
                                <div class="absolute -right-2 -top-2 flex gap-1 z-10">
                                    <span x-show="option.is_correct" class="px-2 py-0.5 bg-emerald-600 text-white text-[8px] font-bold uppercase rounded-md shadow-xs">Kunci</span>
                        ...
                        <template x-for="(option, index) in currentQuestion.options" :key="'mc-'+option.id">
                            <div class="relative flex items-center p-4 rounded-xl border transition-all bg-white border-slate-200"
                                ...
                                <div class="flex items-center gap-1.5 ml-3">
                                    <template x-if="option.is_correct">
                                        <span class="px-2 py-0.5 bg-emerald-50 border border-emerald-250 text-emerald-700 text-[8px] font-extrabold uppercase rounded">Kunci</span>
                                    </template>
                ...
                <template x-if="currentQuestion && currentQuestion.type === 'menjodohkan'">
                    <div class="overflow-hidden border border-slate-200 rounded-xl bg-white shadow-xs">
                        <div class="p-4 bg-slate-50 border-b border-slate-200">
                            <h5 class="text-[10px] font-bold text-slate-500 uppercase tracking-wider">Hasil Menjodohkan</h5>
                        </div>
                            ...
                            <template x-for="option in currentQuestion.options" :key="'match-row-'+option.id">
                                <div class="p-3.5 space-y-2 border border-slate-100 rounded-lg">
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap text-[10px] font-bold">
                                        ...
                                        <div class="px-2 py-0.5 bg-indigo-50 text-indigo-700 border border-indigo-100 rounded">
            ...
            <div class="p-5 bg-slate-900 rounded-xl text-white space-y-3 shadow-md" x-show="currentQuestion">
                ...
                <div class="prose prose-invert prose-sm max-w-none text-slate-300 text-[11px] font-semibold leading-relaxed" x-show="currentQuestion && currentQuestion.explanation" x-html="currentQuestion ? currentQuestion.explanation : ''"></div>
    ...
    <div x-show="showQuestionsDrawer" class="fixed inset-0 z-50 overflow-hidden" style="display: none;">
        ...
        {{-- Sheet --}}
        <div class="fixed bottom-0 inset-x-0 bg-white rounded-t-2xl border-t border-slate-200 max-h-[75vh] flex flex-col z-10 overflow-hidden transform transition-all duration-300">
            ...
            <div class="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-center gap-4 text-[9px] font-bold uppercase tracking-wider text-slate-500">
                <div class="flex items-center gap-1"><span class="w-2.5 h-2.5 rounded bg-emerald-500"></span> Benar</div>
