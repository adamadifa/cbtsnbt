...
<div x-show="showEditModal" x-transition.opacity class="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-sm" x-cloak>
    <div @click.away="showEditModal = false" class="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden border border-slate-100 transform transition-all"
         x-data="{
             ...
             validateName() {
                 this.errors.name = editName.trim() === '' ? 'Nama materi uji wajib diisi' : '';
             },
             validateCode() {
                 this.errors.code = editCode.trim() === '' ? 'Kode materi uji wajib diisi' : '';
             },
             checkSubmit(e) {
        ...
        <div class="bg-slate-50 border-b border-slate-100 px-6 py-5 flex items-center justify-between">
            <div>
                <h3 class="text-base font-bold text-slate-800 tracking-wide">Perbarui Data Materi Uji</h3>
                <p class="text-[11px] text-slate-400 mt-1">Ubah data identitas dan pengaturan materi uji di bawah ini.</p>
            </div>
            ...
            <form :action="editAction" method="POST" @submit="checkSubmit" novalidate class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="relative">
                        <div class="group border rounded-xl px-3 py-1.5 flex items-center gap-2 transition-all duration-200"
                            ...
                            <label for="edit_code" class="absolute -top-2.5 left-3 px-1.5 bg-white text-[10px] font-bold transition-all duration-200"
                            ...
                            <input id="edit_code" name="code" type="text" required
                                   x-model="editCode"
                                   @input="validateCode"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="group relative border border-slate-200 focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-100/50 rounded-xl px-3 py-1.5 flex items-center gap-2 transition-all">
                        <label for="edit_component" class="absolute -top-2.5 left-3 px-1.5 bg-white text-[10px] font-bold text-slate-500 group-focus-within:text-orange-600 transition-all duration-200">Komponen</label>
                        <i class="ti ti-category text-slate-400 shrink-0 text-base"></i>
                        <select id="edit_component" name="component" x-model="editComponent" class="w-full bg-transparent border-0 p-0 text-xs text-slate-600 focus:ring-0 focus:outline-none py-1">
                    ...
                    <!-- Order -->
                    <div class="group relative border border-slate-200 focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-100/50 rounded-xl px-3 py-1.5 flex items-center gap-2 transition-all">
                        <label for="edit_order" class="absolute -top-2.5 left-3 px-1.5 bg-white text-[10px] font-bold text-slate-500 group-focus-within:text-orange-600 transition-all duration-200">Urutan Tampilan</label>
                        <i class="ti ti-sort-ascending-numbers text-slate-400 shrink-0 text-base"></i>
                        <input id="edit_order" name="order" type="number" x-model="editOrder"
                               class="w-full bg-transparent border-0 p-0 text-xs text-slate-800 placeholder-slate-300 focus:ring-0 focus:outline-none py-1" />
                    </div>
                </div>
                ...
                <div class="group relative border border-slate-200 focus-within:border-orange-500 focus-within:ring-2 focus-within:ring-orange-100/50 rounded-xl px-3 py-1.5 flex items-start gap-2 transition-all">
                    <label for="edit_description" class="absolute -top-2.5 left-3 px-1.5 bg-white text-[10px] font-bold text-slate-500 group-focus-within:text-orange-600 transition-all duration-200">Deskripsi</label>
                    <i class="ti ti-notes text-slate-400 shrink-0 text-base mt-1"></i>
                    <textarea id="edit_description" name="description" rows="3" x-model="editDescription"
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex flex-wrap items-center gap-4 py-1.5">
                        ...
                        @foreach ($colors as $name => $hex)
                            <button type="button" @click="editColor = '{{ $hex }}'" 
                                    class="w-8 h-8 rounded-xl border-2 transition-all hover:scale-110 flex items-center justify-center" 
                                    :class="editColor === '{{ $hex }}' ? 'border-orange-500 scale-105 shadow-sm ring-2 ring-orange-200' : 'border-transparent'"
                                    style="background-color: {{ $hex }}">
                                <span x-show="editColor === '{{ $hex }}'" class="text-white text-[10px]">✓</span>
                            </button>
                        @endforeach
                    </div>
                </div>
