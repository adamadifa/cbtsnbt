...
@section('page_title', 'Perbarui Sesi Ujian')

@section('content')
<div class="max-w-7xl mx-auto" x-data="sessionForm()">
    <form action="{{ route('admin.exam-sessions.update', $examSession) }}" method="POST" @submit="submitForm($event)" novalidate>
        @csrf @method('PUT')
            ...
            <div class="lg:col-span-3 space-y-5">
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden transition-all hover:shadow-md"
                    ...
                    <div class="p-5 border-b border-slate-50 flex items-center gap-3 bg-slate-50/20">
                        ...
                        <div>
                            <h3 class="text-xs font-bold text-slate-800 uppercase tracking-wider">Perbarui Sesi Ujian</h3>
                            <p class="text-[10px] text-slate-400">Sesuaikan waktu dan identitas sesi ujian</p>
                        </div>
                    </div>
                ...
                <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden transition-all hover:shadow-md">
                    <div class="p-5 border-b border-slate-50 flex items-center gap-3 bg-slate-50/20">
                        ...
                        <div>
                            ...
                            <p class="text-[10px] text-slate-400">Kode akses pengerjaan sesi tryout</p>
                        </div>
                    </div>
                    <div class="p-5">
                        <div class="relative">
...
<script>
    function sessionForm() {
        return {
            title: `{{ addslashes($examSession->title) }}`,
            startTime: `{{ $examSession->start_time->format('Y-m-d H:i') }}`,
            endTime: `{{ $examSession->end_time->format('Y-m-d H:i') }}`,
            token: `{{ $examSession->token }}`,
            selectedPackage: `{{ $examSession->exam_package_id }}`,
            ...
            submitForm(e) {
                ...
                if (!this.title || !this.startTime || !this.endTime || !this.selectedPackage || (this.endTime <= this.startTime)) {
                    e.preventDefault();
