    ...
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            <a href="{{ route('admin.questions.index', ['subject_id' => $question->subject_id]) }}"
                class="text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors flex items-center gap-2 group">
            ...
            <a href="{{ route('admin.questions.edit', $question) }}"
                class="inline-flex items-center gap-2 px-4 py-2 bg-amber-50 hover:bg-amber-100 text-amber-700 text-xs font-semibold rounded-xl border border-amber-200 transition-colors shadow-sm">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-slate-200/70 overflow-hidden">
            <div class="p-6 md:p-8 border-b border-slate-100">
                ...
                <div class="flex flex-wrap items-center gap-2 mb-6">
                    <span
                        class="px-2.5 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-indigo-100/50">
                        {{ str_replace('_', ' ', $question->type) }}
                    </span>

                    @if($question->difficulty === 'mudah')
                        <span
                            class="px-2.5 py-1 bg-emerald-50 text-emerald-700 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-emerald-100/50">
                            Kesulitan: Mudah
                        </span>
                    @elseif($question->difficulty === 'sedang')
                        <span
                            class="px-2.5 py-1 bg-amber-50 text-amber-700 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-amber-100/50">
                            Kesulitan: Sedang
                        </span>
                    @else
                        <span
                            class="px-2.5 py-1 bg-rose-50 text-rose-700 text-[10px] font-bold uppercase tracking-wider rounded-lg border border-rose-100/50">
                            Kesulitan: Sulit
                        </span>
                    @endif
                </div>
                ...
                @if($question->options->count() > 0)
                            ...
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="bg-slate-50 border-b border-slate-200">
                                        <th class="px-5 py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider w-1/2">
                                            Item Kiri (Premis)</th>
                                        <th class="px-5 py-3 text-[10px] font-bold text-indigo-700 uppercase tracking-wider w-1/2">
                                            Item Kanan (Kunci Pasangan)</th>
                                    </tr>
                                </thead>
                    ...
                    @else
                        <div class="space-y-3">
                            @foreach($question->options as $option)
                                <div
                                    class="flex items-start gap-3.5 p-4 rounded-xl border transition-all duration-200 {{ $option->is_correct ? 'border-emerald-500 bg-emerald-50/20' : 'border-slate-200/80 bg-white hover:border-slate-300' }}">
                                    <div class="flex-shrink-0">
                                        <div
                                            class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-bold transition-colors {{ $option->is_correct ? 'bg-emerald-600 text-white shadow-sm' : 'bg-slate-50 border border-slate-200 text-slate-500' }}">
                                            {{ $option->label }}
                                        </div>
                                    </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div
                class="bg-white p-5 rounded-2xl border border-slate-200/70 shadow-sm flex items-center gap-4 hover:border-slate-350 transition-colors duration-200">
                ...
                <div>
                    ...
                    <p class="text-xs font-bold text-slate-800 mt-0.5">
                        {{ $question->created_at->translatedFormat('d M Y, H:i') }}</p>
                </div>
            </div>
