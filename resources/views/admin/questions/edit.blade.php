...
<div class="max-w-7xl mx-auto" x-data="questionForm()">
    ...
    <div class="mb-6 flex items-center justify-between">
        <a href="{{ route('admin.questions.index', ['subject_id' => $question->subject_id]) }}" class="inline-flex items-center gap-2 text-xs font-semibold text-slate-500 hover:text-indigo-600 transition-colors group">
    ...
    <form action="{{ route('admin.questions.update', $question) }}" method="POST" id="main-form" class="grid grid-cols-1 lg:grid-cols-4 gap-6" @submit="submitForm($event)" novalidate>
        @csrf @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden transition-all duration-300"
                ...
                <div class="p-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/40">
                    <div class="flex items-center gap-3.5">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-slate-800 tracking-tight">Perbarui Konten Soal <span class="text-red-500">*</span></h3>
                            <p class="text-xs text-slate-400 mt-0.5">ID Soal: #{{ $question->id }}</p>
                        </div>
                    </div>
                </div>
                <div class="p-6">
                    <div class="rounded-xl border border-slate-200 overflow-hidden bg-white focus-within:ring-2 focus-within:ring-indigo-100 focus-within:border-indigo-500 transition-all duration-200">
                        <textarea name="content" id="editor-main" class="hidden">{{ old('content', $question->content) }}</textarea>
                    </div>
                    <template x-if="touched.content && !hasContent">
                        <p class="text-xs font-medium text-red-600 mt-2 ml-1 animate-in fade-in slide-in-from-top-1">Konten soal wajib diisi</p>
                    </template>
                </div>
            </div>

            {{-- Options Card --}}
            <div x-show="type !== 'essai'" x-transition class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden transition-all duration-300">
                {{-- Options Header --}}
                <div class="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/40" x-show="type === 'pilihan_ganda' || type === 'pilihan_ganda_kompleks'">
                    <div class="flex items-center gap-3.5">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-slate-800 tracking-tight">Pilihan Jawaban <span class="text-red-500">*</span></h3>
                            <p class="text-xs text-slate-400 mt-0.5">Tentukan opsi jawaban dan tandai opsi yang benar</p>
                        </div>
                    </div>
                    <button type="button" @click="addOption()" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-xs font-semibold rounded-lg transition-colors group">
                ...
                <div class="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/40" x-show="type === 'menjodohkan'">
                    <div class="flex items-center gap-3.5">
                        <div class="w-9 h-9 bg-indigo-50 rounded-xl flex items-center justify-center text-indigo-600">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div>
                            <h3 class="text-sm font-semibold text-slate-800 tracking-tight">Pasangan Menjodohkan <span class="text-red-500">*</span></h3>
                            <p class="text-xs text-slate-400 mt-0.5">Hubungkan item kiri dengan jawaban benar di kanan</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="p-6 space-y-5">
                                ...
                                <div class="flex flex-col gap-2">
                                    <div class="flex gap-4 p-4 rounded-xl border border-slate-200/70 bg-white group relative hover:border-indigo-200 hover:shadow-sm transition-all duration-300"
                                        ...
                                        <div class="flex-grow max-w-[calc(100%-110px)]">
                                            <div class="rounded-lg border border-slate-200 focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-50 overflow-hidden transition-all bg-white">
                                                <textarea :id="`editor-opt-${option.id}`" 
                                                          :name="`options[${option.label}][content]`" 
                                                          x-init="initSummernoteOption($el, index)"
                                                          class="hidden"
                                                          x-text="option.content"></textarea>
                                            </div>
                                        </div>

                                        {{-- Remove button --}}
                                        <button type="button" @click="removeOption(index)" x-show="options.length > 2" class="flex-shrink-0 p-2 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all h-fit self-center">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <template x-if="touched.options && !option.hasContent">
                                        <p class="text-[11px] font-medium text-red-600 ml-16 animate-pulse">Konten untuk pilihan <span x-text="option.label"></span> wajib diisi</p>
                                    </template>
                                </div>
                            </template>
                        </div>
                    ...
                    <template x-if="type === 'menjodohkan'">
                        <div class="space-y-4">
                            <template x-for="(pair, index) in matchingPairs" :key="pair.id">
                                <div class="p-5 rounded-xl border border-slate-200 bg-slate-50/20 space-y-4 relative group hover:border-slate-300 hover:bg-white transition-all duration-300">
                                    <div class="flex items-center justify-between">
                                        <div class="flex items-center gap-2">
                                            <div class="w-6 h-6 flex items-center justify-center bg-indigo-50 border border-indigo-100 rounded-lg text-xs font-bold text-indigo-600" x-text="index + 1"></div>
                                            <span class="text-xs font-semibold text-slate-600">Pasangan Ke-<span x-text="index + 1"></span></span>
                                        </div>
                                        <button type="button" @click="removeMatchPair(index)" x-show="matchingPairs.length > 1" class="p-1.5 text-slate-400 hover:text-rose-500 hover:bg-rose-50 rounded-lg transition-all">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <div class="space-y-2">
                                            <label class="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Item Kiri (Premis)</label>
                                            <div class="rounded-lg border border-slate-200 overflow-hidden focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-50 transition-all bg-white">
                                                <textarea :name="`match_options[${index}][left]`" 
                                                          x-init="initSummernoteMatch($el, index, 'left')"
                                                          class="hidden"
                                                          x-text="pair.left"></textarea>
                                            </div>
                                        </div>
                                        <div class="space-y-2">
                                            <label class="text-[11px] font-bold text-slate-500 uppercase tracking-wider block">Item Kanan (Kunci)</label>
                                            <div class="rounded-lg border border-slate-200 overflow-hidden focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-50 transition-all bg-white">
                                                <textarea :name="`match_options[${index}][right]`" 
                                                          x-init="initSummernoteMatch($el, index, 'right')"
                                                          class="hidden"
                                                          x-text="pair.right"></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </template>
                            <button type="button" @click="addMatchPair()" class="w-full py-3.5 border border-dashed border-slate-300 rounded-xl text-slate-500 text-xs font-semibold hover:border-indigo-400 hover:text-indigo-600 hover:bg-indigo-50/20 transition-all flex items-center justify-center gap-2 group">
                                <svg class="w-4 h-4 group-hover:rotate-90 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 4v16m8-8H4"></path>
                                </svg>
                    ...
                    <template x-if="touched.correctSelection && !hasCorrectSelection">
                        <div class="p-4 bg-red-50 border border-red-100 rounded-xl flex items-center gap-3 mt-4">
                            ...
                            <p class="text-xs font-semibold text-red-600">Pilih setidaknya satu opsi sebagai jawaban yang benar!</p>
                        </div>
                    </template>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden transition-all duration-300">
                <div class="p-5 border-b border-slate-100 flex items-center gap-3.5 bg-slate-50/40">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-400 mt-0.5">Berikan ulasan jawaban lengkap untuk ditampilkan pasca ujian</p>
                    </div>
                </div>
                <div class="p-6">
                    <div class="rounded-xl border border-slate-200 overflow-hidden bg-white focus-within:ring-2 focus-within:ring-indigo-100 focus-within:border-indigo-500 transition-all duration-200">
                        <textarea name="explanation" id="editor-explanation" class="hidden">{{ old('explanation', $question->explanation) }}</textarea>
                    </div>
                </div>
            </div>
        </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/70 shadow-sm overflow-hidden sticky top-6">
                ...
                <div class="p-5 space-y-5">
                    ...
                    <div class="space-y-1.5" x-data="{ localTouched: false }">
                        <label class="text-xs font-semibold text-slate-600">Mata Pelajaran <span class="text-red-500">*</span></label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5">
                            ...
                            <input type="number" name="points" step="0.5" x-model="points" class="w-full border-slate-200 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-indigo-100 focus:border-indigo-500 py-2.5 bg-slate-50/30 text-center">
                        </div>
                        <div class="space-y-1.5">
                            <label class="text-xs font-semibold text-slate-600">Poin Salah</label>
                            <input type="number" name="negative_points" step="0.5" x-model="negative_points" class="w-full border-slate-200 rounded-xl text-xs font-semibold focus:ring-2 focus:ring-rose-100 focus:border-rose-400 py-2.5 bg-slate-50/30 text-center text-rose-600">
                        </div>
                    </div>

                    {{-- Actions --}}
                    <div class="pt-4 border-t border-slate-100 space-y-2">
                        <button type="submit" class="w-full py-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold uppercase tracking-wider shadow-sm hover:shadow transition-all flex items-center justify-center gap-2">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24 animate-pulse">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M5 13l4 4L19 7"></path>
                            </svg>
                            Perbarui Soal
                        </button>
                        <a href="{{ route('admin.questions.index') }}" class="w-full py-3 bg-slate-50 hover:bg-slate-100 text-slate-500 text-center rounded-xl text-xs font-bold uppercase tracking-wider transition-all block">
                            Batal
                        </a>
                    </div>
                </div>
...
<script>
    ...
    function questionForm() {
        return {
            subject_id: '{{ old('subject_id', $question->subject_id) }}',
            type: '{{ old('type', $question->type) }}',
            difficulty: '{{ old('difficulty', $question->difficulty) }}',
            points: '{{ old('points', $question->points) }}',
            negative_points: '{{ old('negative_points', $question->negative_points) }}',
            hasContent: true,
